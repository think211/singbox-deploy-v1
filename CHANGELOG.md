...
- 加固 VLESS + Reality + XTLS Vision 部署稳定性。
- 默认 Reality SNI 从 `www.apple.com` 改为 `apple.com`。
- 将 `jq` 补入必需依赖，避免 JSON 管理操作缺依赖。
- sing-box 内核更新改为只更新到脚本验证过的 pinned 稳定版本，不再追 GitHub latest。
- 服务端 VLESS inbound 补回 `sniff` 和 `sniff_override_destination`。
- direct outbound 根据 IPv4/IPv6 检测结果写入 `domain_strategy`。
- 当前脚本仍只部署唯一协议：VLESS + Reality + XTLS Vision，不启用 Hysteria2。
